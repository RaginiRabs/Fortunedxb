...
export default function StepMedia() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // SEO Validation - Case insensitive and trimmed
      const newErrors = {};

      if (formData.seo_city) {
        const normalizedSeoCity = normalizeString(formData.seo_city);
        const normalizedCity = normalizeString(formData.city);

        if (normalizedSeoCity !== normalizedCity) {
          newErrors.seo_city = 'City must match project city';
          setSubmitError(`SEO city "${formData.seo_city}" does not match project city "${formData.city}"`);
        }
      }

      const developerName = getDeveloperName();
      if (formData.seo_developer_name) {
        const normalizedSeoDev = normalizeString(formData.seo_developer_name);
        const normalizedDev = normalizeString(developerName);

        if (normalizedSeoDev !== normalizedDev) {
          newErrors.seo_developer_name = 'Developer name must match selected developer';
          setSubmitError(`SEO developer name "${formData.seo_developer_name}" does not match selected developer "${developerName}"`);
        }
      }
      ...
      } else if (files.remove_logo) {
        submitFormData.append('remove_logo', 'true');
      }
