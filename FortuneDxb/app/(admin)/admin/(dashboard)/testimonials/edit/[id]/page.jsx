...
import { useTestimonial, useUpdateTestimonial } from '@/hooks/testimonial/useTestimonialsHook';
...
export default function EditTestimonialPage() {
  ...
  const { testimonial, loading: fetchLoading, fetchTestimonial } = useTestimonial(testimonialId);
  ...
  // Fetch testimonial and projects on mount
  useEffect(() => {
    if (testimonialId) {
  ...
  useEffect(() => {
    if (testimonial && !isLoaded) {
      setFormData({
        ...
        client_location: testimonial.client_location || 'Dubai, UAE',
        client_image: null,
        client_image_preview: testimonial.client_image || '',
  ...
  const handleSubmit = async () => {
    ...
    const result = await updateTestimonial(testimonialId, formDataToSend);
  ...
  // Loading skeleton
  if (fetchLoading || !isLoaded) {
    return (
      <Box>
        {/* <PageHeader
          title="Edit Testimonial"
          subtitle="Update client testimonial"
          backButton={
            <Button
  ...
  return (
    <Box>
      ...
      {/* <PageHeader
        title="Edit Testimonial"
        subtitle={`Editing: ${formData.client_name}`}
        backButton={
          <Button
            startIcon={<ArrowBackOutlined />}
            onClick={handleCancel}
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={{xs: 12, sm: 6 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Rating
                ...
                emptyIcon={<StarOutlined sx={{ color: 'divider' }} />}
              />
          ...
          <Grid size={{xs: 12, sm: 6 }}>
            <FileUpload
              ...
              existingFile={getImagePath(formData.client_image_preview)}
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 3 }}>
        ...
        <Button
          variant="contained"
          startIcon={updateLoading ? <CircularProgress size={18} sx={{ color: 'common.white' }} /> : <SaveOutlined />}
