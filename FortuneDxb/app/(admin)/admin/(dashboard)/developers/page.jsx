...
export default function DevelopersPage() {
  ...
  const [sortBy, setSortBy] = useState('developer_id');
  ...
  const fetchDevelopers = async () => {
    try {
      ...
      const res = await fetch(
        `/api/developers?limit=${rowsPerPage}&offset=${page * rowsPerPage}&sort_by=${sortBy}&sort_order=${sortOrder}`
      );
      ...
      renderCell: (value, row) => (
        <Avatar
          ...
          sx={{
            ...
            height: 50,
            bgcolor: 'primary.50',
            border: (theme) => `1px solid ${theme.palette.gold.main}`,
            color: 'primary.main',
            fontSize: '1.25rem',
            flexShrink: 0,
            '& img': {
    ...
    <>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          onView={(row) => router.push(`/admin/developers/view/${row.developer_id}`)}
          onEdit={(row) => router.push(`/admin/developers/${row.developer_id}`)}
