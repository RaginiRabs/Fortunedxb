...
export async function GET(request, { params }) {
  try {
    ...
    console.log('RAW PROJECT FROM DB:', {
      ...
      type: typeof project?.handover_date,
    });
    if (!project) {
    ...
    console.log('DB HANDOVER RAW:', project.handover_date);
    console.log('DB HANDOVER ISO:', project.handover_date?.toISOString());
    console.log(
      'DB HANDOVER DATE ONLY:',
      project.handover_date
        ? project.handover_date.toISOString().split('T')[0]
        : null
    );
    ...
    const parsedProject = {
      ...
      handover_date: formatDateLocal(project.handover_date),
      files: {
    ...
    // console.log('FINAL RESPONSE HANDOVER:', parsedProject.handover_date);
...
export async function PUT(request, { params }) {
  try {
    ...
    // Update project - UPDATED with location_iframe and state
    await query(
    ...
    if (projectLogo && projectLogo instanceof File && projectLogo.size > 0) {
      ...
      // Remove existing logo (user clicked remove)
      if (existingProject.project_logo) {
        deleteSingleFile(existingProject.project_logo);
        await query('UPDATE project_details SET project_logo = NULL WHERE project_id = ?', [id]);
      }
    }
