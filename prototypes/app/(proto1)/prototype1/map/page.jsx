...
// prototype1 Dubai market map page — full interactive map with community polygons,
// metro lines, landmarks and project-count badges. Mirrors prototype4. Mock only.
import { useState } from 'react';
import dynamic from 'next/dynamic';
import { MapPin, TrainFront, Landmark, Building2, X } from 'lucide-react';
import { COMMUNITIES } from '@/mock/prototype1/communities';
import { projectsForArea, AREA_PROJECT_CAP } from '@/mock/prototype1/areaProjects';
...
export default function Prototype1MapPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 md:px-8">
      {/* Heading */}
      <span className="block mb-1.5 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#80603f]">Dubai —</span>
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        {selected && (
          <div className="rounded-xl border border-[#e8e2da] bg-[#faf7f3] px-5 py-3">
            ...
            <p className="flex items-center gap-2 text-lg font-bold text-[#1a1a1a]">
          ...
          <div className="flex h-full flex-col overflow-hidden rounded-l-2xl border border-[#e8e2da] bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-3 border-b border-[#e8e2da] bg-[#faf7f3] px-5 py-4">
              <div className="min-w-0">
                ...
                <h3 className="truncate text-lg font-bold text-[#1a1a1a]">
                  {drawer?.name}
                </h3>
              ...
              <button
                ...
                aria-label="Close"
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-[#675c4e] transition-colors hover:bg-[#f0ebe3] hover:text-[#1a1a1a]"
              >
                <X size={18} />
              </button>
            </div>
