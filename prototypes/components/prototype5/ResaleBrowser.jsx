...
// prototype5 resale listings browser — functional filters, sort, pagination, grid/list. Mock only.
          ...
          {shown.map((p, idx) => (
            <article key={`${p.name}-${idx}`} className={`group overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-[0_12px_34px_-16px_rgba(20,18,15,0.2)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_22px_46px_-18px_rgba(128,96,63,0.28)] ${view === 'list' ? 'flex' : ''}`}>
              ...
              <div className="flex-1 p-4">
                ...
                <a href="/prototype5/project/one-by-nine" className={`mt-3 flex items-center justify-center gap-1.5 rounded-lg bg-gradient-to-r from-[#96714a] to-[#6b4f33] px-4 py-2.5 text-[12px] font-medium text-white transition-all hover:brightness-105 ${view === 'list' ? 'w-fit' : 'w-full'}`}>View Property <ArrowRight className="h-3.5 w-3.5" /></a>
              </div>
            </article>
          ))}
        </div>
